...
const Shorten = () => {
  ...
  const handleCopy = () => {
    ...
    toast.success("🚀 Copied! Faster than your last breakup.");
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 pt-16 pb-1
    8">
      <form
      ...
      {shortUrl && !shortUrl.startsWith("Error:") && (
        <div className="mt-4 bg-green-100 border border-green-400 text-green-800 p-3 rounded w-full max-w-lg flex justify-between items-center">
          ...
          <button
            onClick={handleCopy}
            className="ml-3 p-2 hover:bg-green-200 rounded"
            title="Copy"
          >
            <ClipboardCopy className="text-blue-700 h-5 w-5" />
          </button>
        </div>
      )}
      ...
      {/* Link to Dashboard */}
      <div className="mt-3 w-full max-w-lg text-center">
        <Link
          to="/dashboard"
